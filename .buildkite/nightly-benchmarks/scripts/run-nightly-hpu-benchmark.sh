...
check_gpus() {
    ...
    if [[ $gpu_count -gt 0 ]]; then
        ...
        echo "Need at least 1 HPU to run benchmarking."
        exit 1
    fi

    # Get HPU type (assuming 'Habana Gaudi' is the model output by hl-smi)
    declare -g gpu_type=$(hl-smi hl-smi --query-aip=name --format=csv,nounits,noheader  | head -n 1)  # Extracts the model type.
    echo "HPU type is $gpu_type"
}
...
run_serving_tests() {
  ...
  jq -c '.[]' "$serving_test_file" | while read -r params; do
    ...
    if [[ $reuse_server == "true" ]]; then
      ...
      kill_gpu_processes
      echo "Running command: bash $VLLM_SOURCE_CODE_LOC/.buildkite/nightly-benchmarks/scripts/launch-server.sh \"$server_params\" \"$common_params\""
